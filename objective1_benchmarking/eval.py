...
def setup_ddp():
    ...
    # If launched with torchrun, these env vars are present
    launched = all(k in os.environ for k in ["RANK", "WORLD_SIZE", "LOCAL_RANK"])
    if launched:
        dist.init_process_group(backend=backend)
        local_rank = int(os.environ.get("LOCAL_RANK", 0))
    else:
        # Single-process (no torchrun): init a 1-process group
        dist.init_process_group(
            backend=backend,
            init_method="file:///tmp/llada_eval_pg",
            rank=0,
            world_size=1,
        )
        local_rank = 0

    # Only set CUDA device if a GPU is available
...
def evaluate(
    ...
    for batch in tqdm(dataloader, disable=(dist.is_initialized() and dist.get_rank() != 0)):
        start_time = time.time()
        input_ids = batch["input_ids"].to(device)
        gt_answers = batch["answers"]
        questions = batch["questions"]
