...
def get_dataset(dataset_name, tokenizer, few_shot, add_reasoning, subsample, train_jsonl=None):
    """
    Returns (dataset, collate_fn). Tries known dataset wrappers from the repo.
    """
    if dataset_name == "gsm8k":
        from gsm8k import GSM8KDataset
        ds = GSM8KDataset(tokenizer, subsample=subsample, num_examples=few_shot, add_reasoning=add_reasoning)
        return ds, ds.collate_fn
    if dataset_name == "math":
        from math500 import MATH500Dataset
        ds = MATH500Dataset(tokenizer, subsample=subsample, num_examples=few_shot, add_reasoning=add_reasoning)
        return ds, ds.collate_fn
    if dataset_name == "countdown":
        from countdown import CTDDataset
        ds = CTDDataset(tokenizer, subsample=subsample, num_examples=few_shot, add_reasoning=add_reasoning)
        return ds, ds.collate_fn
    if dataset_name == "sudoku":
        from sudoku import SudokuDataset
        ds = SudokuDataset(tokenizer, subsample=subsample, num_examples=few_shot, add_reasoning=add_reasoning)
        return ds, ds.collate_fn

    if dataset_name == "logiqa":
        from logiqa import LogiQADataset
        ds = LogiQADataset(tokenizer, subsample=subsample, num_examples=few_shot, add_reasoning=add_reasoning)
        return ds, ds.collate_fn

    if dataset_name in ("gpqa", "gpqa_ext", "gpqa_diamond"):
        from gpqa_diamond import GPQADiamondDataset as GPQAClass
        # from gpqa_diamond_2 import GPQADiamondArrowDataset as GPQAClass
        ds = GPQAClass(tokenizer, subsample=subsample, num_examples=few_shot,
                       add_reasoning=add_reasoning, train_jsonl=train_jsonl)
        return ds, ds.collate_fn

    if dataset_name == "aime2025":
        from aime2025 import AIME2025Dataset
        ds = AIME2025Dataset(tokenizer, subsample=subsample, num_examples=few_shot, add_reasoning=add_reasoning)
        return ds, ds.collate_fn

    raise ValueError(f"Unknown dataset '{dataset_name}'. Add a wrapper or change --dataset.")
...
class CustomDistributedSampler(DistributedSampler):
    ...
    divisible across the number of replicas. If False, the sampler will add extra indices to make the data
    ...
    def __init__(
        ...
        if self.drop_last and len(self.dataset) % self.num_replicas != 0:
            self.num_samples = int((len(self.dataset) - self.num_replicas) / self.num_replicas)
...
def jsonl_count_valid_lines(path: str) -> int:
    """Count parseable JSON lines; tolerate trailing partial lines."""
    ...
    with open(path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            try:
                ...
                # ignore malformed line (e.g., crash mid-write)
                break
    return n

def jsonl_append_many(path: str, records):
    """Append a list of dicts to JSONL (fsync for safety)."""
...
def jsonl_load_all(path: str):
    """Load all valid JSON lines; skip malformed trailing line."""
    out = []
    if not os.path.exists(path):
        return out
    with open(path, "r", encoding="utf-8") as f:
...
def evaluate(model,
    ...
    # If we already streamed some lines, do NOT re-skip them here — main() passes the right skip_examples
    already_streamed = jsonl_count_valid_lines(jsonl_path) if jsonl_path else 0
    ...
    for bidx, batch in enumerate(tqdm(dataloader, disable=(get_rank() != 0),
        ...
        # ---- STREAM APPEND to JSONL immediately ----
        if jsonl_path:
    ...
    # We will rebuild final JSON from JSONL outside; return minimal metrics here.
    return {
...
def main():
    ...
    parser.add_argument("--dataset", type=str,
                        choices=["gsm8k", "math", "countdown", "sudoku", "logiqa", "gpqa", "gpqa_ext", "gpqa_diamond", "aime2025"],
                        default="gsm8k")
    ...
    parser.add_argument("--no_finalize", action="store_true", help="Do not write the final *_generations.json bundle.")

    parser.add_argument("--train_jsonl", type=str, default=None)

    # fallback/cache knobs (unchanged)
    ...
    # per the repo, negative subsample means "use all"
    num_evals = {
        "gsm8k": -1, "math": -1, "countdown": 256, "sudoku": 256,
        "logiqa": -1, "gpqa": -1, "gpqa_ext": -1, "gpqa_diamond": -1, "aime2025": -1,
    }

    dataset, collate = get_dataset(
        args.dataset, tokenizer, args.few_shot, add_reasoning=True,
        subsample=num_evals[args.dataset], train_jsonl=args.train_jsonl
    )

    total_len = len(dataset)
    rank = get_rank()
    if rank == 0:
        print(f"evaluating {total_len} examples")
    ...
    # show expectations up front
    remaining = total_len - start_idx
    bs = max(1, int(args.batch_size))
    expected_batches = (remaining + bs - 1) // bs
    if rank == 0:
        print(f"[resume] continuing {args.dataset} from index {start_idx} "
    ...
    # slice dataset if single-rank resume; IMPORTANT: when we slice, we must NOT skip in evaluate
    ...
    if not use_ddp and start_idx > 0:
        ...
        sampler = CustomDistributedSampler(dataset, shuffle=False) if use_ddp else None
        skip_examples = start_idx   # skip within evaluate over the full dataset
        state_base    = start_idx
    ...
    out_json = f"{args.output_dir}/{args.dataset}_{model_name}_{args.gen_length}_{args.diffusion_steps}_{rank}_generations.json"
    if rank == 0:
    ...
    if rank == 0 and not args.dont_save and not args.no_finalize:
        ...
        # keep state.json so you can inspect progress; delete manually if desired
